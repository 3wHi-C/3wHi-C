...
cd /mnt/disk6/3/software/ChromatinImaging-master/Data

for i in `cat cell_ID`
do
	awk -F ',' '{if($1==a)print $2-1","$3-1}' a=$i K562_dis_150nm.cliques > cliques.${i}.cells
	...
	cat final_gt3_${i}_cliques2 | awk -F " " -v a=${i} '{print a" "$0}' >> total_cell_cliques
